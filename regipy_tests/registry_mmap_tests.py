...
def test_parse_header(ntuser_hive):
    registry_hive = MemoryMappedRegistryHive(ntuser_hive)

    assert isinstance(registry_hive, MemoryMappedRegistryHive)
...
def test_find_keys_partial_ntuser_hive(ntuser_software_partial):
    registry_hive = MemoryMappedRegistryHive(ntuser_software_partial, hive_type=NTUSER_HIVE_TYPE, partial_hive_path=r'\Software')
...
def test_regdiff(ntuser_hive, second_hive_path):
    found_differences = compare_hives(ntuser_hive, second_hive_path, verbose=True, use_memory_map=True)
...
def test_ntuser_emojis(transaction_ntuser):
    ...
    registry_hive = MemoryMappedRegistryHive(transaction_ntuser)
...
def test_recurse_amcache(amcache_hive):
    registry_hive = MemoryMappedRegistryHive(amcache_hive)
...
def test_system_hive_devprop_structure(system_devprop):
    registry_hive = MemoryMappedRegistryHive(system_devprop)
...
def test_get_key(software_hive):
    ...
    """
    registry_hive = MemoryMappedRegistryHive(software_hive)
...
def test_parse_filetime_value(system_hive_with_filetime):
    registry_hive = MemoryMappedRegistryHive(system_hive_with_filetime)
