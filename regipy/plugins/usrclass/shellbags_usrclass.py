...
class ShellBagUsrclassPlugin(Plugin):
    ...
    @staticmethod
    def _get_shell_item_description(shell_item):
        description = shell_folders.WindowsShellFoldersHelper.GetDescription(shell_item.shell_folder_identifier)
        if not description and shell_item.shell_folder_identifier in known_guids:
            description = known_guids[shell_item.shell_folder_identifier]

        if not description and hasattr(shell_item, 'identifier'):
            description = shell_folders.WindowsShellFoldersHelper.GetDescription(shell_item.identifier)
            if not description and shell_item.identifier in known_guids:
                description = known_guids[shell_item.identifier]
        return description
    ...
    def _parse_shell_item_path_segment(self, shell_item):
        ...
        if isinstance(shell_item, pyfwsi.volume):
            if shell_item.name:
                ...
                path_segment = shell_folders.WindowsShellFoldersHelper.GetDescription(shell_item.identifier)
                if not path_segment and shell_item.identifier in known_guids:
            ...
            description = self._get_shell_item_description(shell_item)
